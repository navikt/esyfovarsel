...
| Mikrofronter på Min side         | `aktivitetskrav-backend`<br>`isdialogmote`<br>`ismeroppfolging`<br>`meroppfolging-backend`<br>`syfomotebehov`                                                                   | Publisering av `enable` og `disable` til `min-side.aapen-microfrontend-v1`, og egen logikk for synlighet, utløp og mapping til `syfo-dialog`, `syfo-aktivitetskrav` og `syfo-meroppfolging`. |

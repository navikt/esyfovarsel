...
class SyketilfelleKafkaConsumer(
    ...
    override suspend fun listen(applicationState: ApplicationState) {
        log.info("[FLEX]: Starting to listen to $topicFlexSyketilfellebiter")
        while (applicationState.running) {
            kafkaListener.poll(zeroMillis).forEach {
                log.info("[FLEX]: Received record from [$topicFlexSyketilfellebiter]")
                try {
                    ...
                    log.error(
                        "[FLEX]: Error in [$topicFlexSyketilfellebiter] listener: Could not parse message | ${e.message}",
                        e
                    )
                }
            }
